...
from typing import Any, Awaitable, Mapping, Sequence, TypeVar, cast
...
def test_pubmed_adapter_parses_fixture(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    async def fake_fetch_json(
        ...
        *,
        ...
        if "esearch" in url:
            return cast(dict[str, object], json.loads(Path("tests/fixtures/ingestion/pubmed_search.json").read_text()))
        return cast(dict[str, object], json.loads(Path("tests/fixtures/ingestion/pubmed_summary.json").read_text()))
    ...
    async def _exec() -> None:
        results = cast(list[IngestionResult], list(await adapter.run(term="lactate")))
        assert len(results) == 1
        assert results[0].document.metadata["title"].startswith("Lactate")
        raw_doc = cast(dict[str, Any], results[0].document.raw)
        assert raw_doc["pmcid"] == "PMC1234567"
        assert "Sepsis" in cast(list[str], raw_doc["mesh_terms"])
        assert raw_doc["doi"] == "10.1000/abc123"
        authors = cast(list[str], raw_doc["authors"])
        assert any("Jane Smith" in author for author in authors)
        await client.aclose()
...
def test_clinicaltrials_adapter_validates_nct(tmp_path: Path) -> None:
    ...
    context = AdapterContext(ledger=ledger)
    client = AsyncHttpClient()
    payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/ctgov_study.json").read_text()))
    adapter = ClinicalTrialsGovAdapter(context, client, bootstrap_records=[payload])

    async def _exec() -> None:
        results = cast(list[IngestionResult], list(await adapter.run()))
...
def test_openfda_adapter_handles_identifier(tmp_path: Path) -> None:
    ...
    context = AdapterContext(ledger=ledger)
    client = AsyncHttpClient()
    openfda_raw = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/openfda_faers.json").read_text()))
    payload = cast(dict[str, Any], openfda_raw["results"][0])
    adapter = OpenFdaAdapter(context, client, bootstrap_records=[payload])

    async def _exec() -> None:
        results = cast(list[IngestionResult], list(await adapter.run(resource="drug/event")))
...
def test_terminology_adapters(tmp_path: Path) -> None:
    ...
    context = AdapterContext(ledger=ledger)
    client = AsyncHttpClient()
    mesh_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/mesh_descriptor.json").read_text()))
    umls_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/umls_cui.json").read_text()))
    loinc_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/loinc_lookup.json").read_text()))
    icd_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/icd11_code.json").read_text()))
    snomed_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/snomed_lookup.json").read_text()))

    async def _exec() -> None:
        mesh_result = cast(
            list[IngestionResult],
            list(await MeSHAdapter(context, client, bootstrap_records=[mesh_payload]).run(descriptor_id="D012345")),
        )
        umls_result = cast(
            list[IngestionResult],
            list(await UMLSAdapter(context, client, bootstrap_records=[umls_payload]).run(cui="C1234567")),
        )
        loinc_result = cast(
            list[IngestionResult],
            list(await LoincAdapter(context, client, bootstrap_records=[loinc_payload]).run(code="4548-4")),
        )
        icd_result = cast(
            list[IngestionResult],
            list(await Icd11Adapter(context, client, bootstrap_records=[icd_payload]).run(code="1A00")),
        )
        snomed_result = cast(
            list[IngestionResult],
            list(await SnomedAdapter(context, client, bootstrap_records=[snomed_payload]).run(code="44054006")),
        )
...
def test_guideline_adapters(tmp_path: Path) -> None:
    ...
    context = AdapterContext(ledger=ledger)
    client = AsyncHttpClient()
    nice_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/nice_guideline.json").read_text()))
    cdc_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/cdc_socrata.json").read_text()))
    ...
    async def _exec() -> None:
        nice_result = cast(list[IngestionResult], list(await adapter_nice.run()))
        cdc_result = cast(list[IngestionResult], list(await adapter_cdc.run(dataset="abc")))
...
def test_accessgudid_validation(tmp_path: Path) -> None:
    ...
    context = AdapterContext(ledger=ledger)
    client = AsyncHttpClient()
    payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/accessgudid.json").read_text()))
    adapter = AccessGudidAdapter(context, client, bootstrap_records=[payload])

    async def _exec() -> None:
        results = cast(list[IngestionResult], list(await adapter.run(udi_di="00380740000011")))
...
def test_literature_adapters(tmp_path: Path) -> None:
    ...
    medrxiv_payload = cast(dict[str, Any], json.loads(Path("tests/fixtures/ingestion/medrxiv.json").read_text()))
    ...
    async def _exec() -> None:
        pmc_results = cast(list[IngestionResult], list(await pmc_adapter.run(set_spec="pmc")))
        medrxiv_results = cast(list[IngestionResult], list(await medrxiv_adapter.run()))
