...
def _candidate(identifier: str, score: float) -> Candidate:
    return Candidate(
        identifier=identifier, ontology="rxnorm", score=score, label=identifier, metadata={}
    )
...
def test_decision_accepts_high_confidence_llm() -> None:
    ...
    llm = AdjudicationResult(
        chosen_id="rx1", ontology="rxnorm", score=0.9, evidence_span={}, alternates=[], notes=None
    )
...
def test_decision_falls_back_to_deterministic_identifier() -> None:
    ...
    llm = AdjudicationResult(
        chosen_id=None, ontology=None, score=0.0, evidence_span={}, alternates=[], notes=None
    )
    identifiers = [
        IdentifierCandidate(scheme="RxCUI", code="1234", confidence=0.95, start=0, end=4)
    ]
