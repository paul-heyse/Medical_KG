    ...
    *,
    ...
    for evidence in graph.get("evidence", []):
        ...
        if unit is None or not isinstance(unit, str) or not unit.strip():
            issues.append(
                ShaclIssue(
                    f"Evidence {evidence.get('id')} missing UCUM unit",
                    path=("evidence", str(evidence.get("id"))),
                )
            )
        ...
        if outcome and "outcome" not in evidence:
            issues.append(
                ShaclIssue(
                    f"Evidence {evidence.get('id')} missing outcome node",
                    path=("evidence", str(evidence.get("id")), "outcome"),
                )
            )
    ...
    for adverse_event in graph.get("adverse_events", []):
        ...
        if grade is not None and grade not in {1, 2, 3, 4, 5}:
            issues.append(
                ShaclIssue(
                    f"Adverse event {adverse_event.get('id')} grade invalid",
                    path=("adverse_events", str(adverse_event.get("id"))),
                )
            )
    for node in graph.get("constraints", []):
        if not node.get("generated_by"):
            issues.append(
                ShaclIssue(
                    f"Node {node.get('id')} missing provenance link",
                    path=("constraints", str(node.get("id"))),
                )
            )
...
def _run_shape(
    graph: Mapping[str, Sequence[Mapping[str, object]]], shape: Mapping[str, object]
) -> list[ShaclIssue]:
    issues: list[ShaclIssue] = []
