...
def license_config(draw) -> tuple[LicenseRegistry, LicenseSession, dict[str, int]]:
    features = draw(
        st.dictionaries(st.text(min_size=1, max_size=5), st.booleans(), min_size=1, max_size=4)
    )
    limits = draw(
        st.dictionaries(
    ...
    registry = LicenseRegistry(
        entries={"VOCAB": LicenseEntry("VOCAB", True, None)}, tiers={"dynamic": tier}
    )
...
def test_usage_limits_respected(
    config: tuple[LicenseRegistry, LicenseSession, dict[str, int]],
) -> None:
...
def test_retention_policy_safety(retention_days: int, age_days: int, dry_run: bool) -> None:
    ...
    record = RetentionRecord(
        doc_id="doc", created_at=now - timedelta(days=age_days), data={"pii": "value"}, tags=()
    )
    pipeline = PurgePipeline({}, {}, {}, {}, {})
    policy = RetentionPolicy(
        name="test", retention_days=retention_days, anonymize_fields=("pii",), dry_run=dry_run
    )
